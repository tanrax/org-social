...
It is heavily inspired by [twtxt](https://twtxt.readthedocs.io/en/stable/) and the extensions developed by the [Yarn community](https://twtxt.dev/). It takes the best of these specifications, eliminates complex parts, leverages Org Mode's native features, and keeps the premise that social networking should be simple, accessible to both humans and machines, and manageable with standard text editing tools.
...
flowchart TB
    ...
    Relay1 --> Push["Org Social Push"]
    Push --> ClientiOS
    Relay1 --> WebReading["Org Social Web Reading"]

    style ClientEmacs fill:#733c9f,color:white
    style ClientiOS fill:#733c9f,color:white
    style Preview fill:#e1f5ff,color:black
    style Push fill:#e1f5ff,color:black
    style WebReading fill:#e1f5ff,color:black
...
- **Org Social Push**: A push notification service that delivers real-time alerts to the iOS client when the Relay detects new mentions, replies, or interactions.

- **Org Social Web Reading**: A public HTML timeline viewer. Reads Org Social feeds through the Relay and renders them as a web page, used for example as an embeddable iframe on [org-social.org](https://org-social.org).
